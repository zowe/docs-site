...
- `zowe.job.name` is the optional customized job name to start Zowe. If not
  defined, the default value is `ZWE1SV`. If it's empty, the start command
  will not pass `JOBNAME=` option to `START` command.
