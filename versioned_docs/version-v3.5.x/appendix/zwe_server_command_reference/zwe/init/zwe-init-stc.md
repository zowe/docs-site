...
- `zowe.setup.dataset.authPluginLib` is the user custom APF LOADLIB for ZIS plugins.
...
zwe init stc --config 'FILE(/path/to/zowe.yaml):PARMLIB(DSN.TO.PARMS(ZWEYAML))' --generate --trace
