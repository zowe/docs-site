...
- Fixed an issue where error message `ZWEL0141E` did not display the user ID, thus improving troubleshooting clarity. ([#3971](https://github.com/zowe/zowe-install-packaging/pull/3971))


### Zowe Application Framework
#### zlux-server-framework
- Fixed an issue where the app-server could not register with the discovery server when AT-TLS was enabled. ([#581](https://github.com/zowe/zlux-server-framework/pull/581))
- Fixed an issue where the app-server /server/environment endpoint was missing the "agent" object, causing the Desktop to use an indirect route to ZSS. This improves latency and high availability behavior for ZSS APIs. ([#589](https://github.com/zowe/zlux-server-framework/pull/589))
- Fixed an issue where troubleshooting information was unavailable when Eureka registration experienced a network failure. Added the components.app-server.node.mediationLayer.traceTls property to help diagnose TLS issues. ([#591](https://github.com/zowe/zlux-server-framework/pull/591))
