...
For example, the sample JCL below shows `ZWESVSTC` where the APF-authorized PDSE containing `ZWESIS01` is `ZWESVUSR.SISLOAD` and the PDS PARMLIB containing `ZWESIP00` is `ZWESVISR.SISSAMP`.  
