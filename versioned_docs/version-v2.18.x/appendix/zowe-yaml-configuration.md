...
- **`apiml.security.forwardHeader.trustedProxies`**
  Specifies the regular expression pattern used to identify trusted proxies from which `X-Forwarded-*` headers are accepted and forwarded. The APIML gateways (incl. cloud gateways) in [Multitenancy Configuration](/user-guide/api-mediation/api-mediation-multi-tenancy) are trusted by default. Applies to version 2.18.2 and later versions.
