...
import { useActiveVersion, useLatestVersion } from "@docusaurus/plugin-content-docs/client"; //Added
...
function NavLink({
  ...
  const latestVersion = useLatestVersion(docsPluginId); //Added
  const activeVersion = useActiveVersion(docsPluginId); //Added
    ...
    <Link
      {...(href
        ...
        : {
            ...
            to: activeVersion
              ? activeVersion.path + toUrl
              : latestVersion.path + toUrl, //Added
