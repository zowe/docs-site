...
## API Mediation Utility and Core Messages (ZWEAM###)
...
Test the z/OSMF JWT endpoint directly using a REST client: 
  ...
  1. **Verify that the response contains a `jwt` field with a valid token string.**
...
**Reason:**  
General exception. There are more pieces of information in the appended message log.
...
**Action:**  
Verify that the secured signon (PassTicket) function and application ID are configured properly by referring to the guide for your specific security provider.
...
**Action:**  
Ensure that the security provider recognizes the application ID.
...
**Action:**  
Ensure that the configuration contains the `trustStorePath` and that it points to a valid trust store.
...
**Action:**  
Ensure that the configuration contains the `keyStorePath` and that it points to a valid key store.

### ZWEAS502E
The configuration provided for SSL is invalid.

**Reason:**  
The type of the keystore, truststore, or the included keys/certs are not considered valid.
...
**Reason:**  
There was an invalid path to either the trust store or keystore.

**Action:**  
Ensure that both provided paths are resolved to a valid trust store and valid key store.
...
## ZAAS Internal Service Messages (ZWESG###)
