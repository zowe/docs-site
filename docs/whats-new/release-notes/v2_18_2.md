...
- The configuration property **`apiml.security.forwardHeader.trustedProxies`** has been added to specify the regular expression pattern used to identify trusted proxies from which `X-Forwarded-*` headers are accepted and forwarded. This mitigates CVE-2025-41235. The APIML gateways (incl. cloud gateways) in [Multitenancy Configuration](/user-guide/api-mediation/api-mediation-multi-tenancy) are trusted by default. ([#4148](https://github.com/zowe/api-layer/pull/4148)) ([#4188](https://github.com/zowe/api-layer/pull/4188))
