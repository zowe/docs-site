...
- Added an enhancement so that the zwe support command collects more environment details. ([#4147]
(https://github.com/zowe/zowe-install-packaging/pull/4147))
- Added a new library function named formatZosVersion(). ([#4134](https://github.com/zowe/zowe-install-packaging/pull/4134))
- Added an enhancement so that when you run the zwe support verify-fingerprints command, you no longer have to input a value such as zowe.yaml as a command-line parameter.

### Zowe Application Framework

- Added an enhancement to compress the Zowe v3 Desktop assets so that the Desktop can load faster on a slow internet connection with a slow network speed. (#658)
- Added an enhancement to validate the certificate properties on startup. (#338)
- Added a utility certificateChecker.js which can use NodeJS to determine if a keystore of PKCS12, PEM, or SAF keyring is valid for use in Zowe. (#597)

### ZSS

- Added a new feature in the form of a modify command to display the module registry information. (#519)
- Added a new feature in the form of a flag to reset the module registry at the cross-memory server start-up. (#519)
- Added an enhancement to include the stub version in the generated HLASM stub. (#743)
- Added an enhancement to expose new cmutils functions. (#740)
- Added the support to pass multiple parameters to ZIS via the START command. (#753, #755)
- Added a parameter to reset the module registry at ZIS start-up. (#754)
- Added an enhancement to make the modregReset function available through dynlink. (#754)
...
- Fixed an issue that prevented the Desktop from selecting the most efficient and direct route to access ZSS. After the fix, the Desktop has the necessary information to identify and use a direct route to access ZSS. (#588)
- Fixed an issue that caused a large memory consumption and a high memory utilization after every startup. The issue was due to a defect in the "axios" library. The functionality of the app-servers does not alter due to this change. (#600)
...
- Allocated storage to be executed with EXECUTABLE=YES. (#507)
- Fixed a leak in the rsusermap code. (#467)
- Enabled the use of single-line WTOs for single-line messages. (#509)
- Ensured that the modreg-based modules are never deleted. (#517, zowe/zss#749)
- Ensured that modreg-based modules are never deleted. (#752, #749)
- Fixed an issue where the schema entry for JWT tracing was being updated with an incorrect name "_zss.jwt". After the fix, the schema entry is updated with the correct name "_zss.jwtTrace", which helps users to identify the log level that can enable JWT tracing. (#757)
- Corrected the modregRegister entry in the dynlink stub. (#754)
- Fixed the incorrect checking of the look-up anchor reset parameter. (#754)
- Fixed an issue where ZSS was not accessible from the Gateway when using AT-TLS. After the fix, ZSS is accessible from the Gateway when you use AT-TLS.
