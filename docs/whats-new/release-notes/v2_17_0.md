...
- `zwe components install` and `zwe components upgrade` now checks the schema validity of any application framework plugins present within a component, so that you will know better if a component is valid prior to running Zowe. ([#3866](https://github.com/zowe/zowe-install-packaging/pull/3866))
...
- The `app-server` configure stage performance increased due to combining two separate processes in this stage `plugins-init.js and initInstance.js` into one. ([#304](https://github.com/zowe/zlux-app-server/pull/304))
- Removed `dns` check specific to `Node 14` and below to reduce startup time. `Node 14` has not been supported since September 2023. ([#304](https://github.com/zowe/zlux-app-server/pull/304))
