...
After contacting Rocket Software, you should receive _zssServer_, _terminal-3270.js_, _rocket-term-handler.js_, and _terminal-vt.js_.
All files must be placed within _zlux-build/externals/Rocket_. zssServer must be placed here on the z/OS host, the others must be placed there wherever the zLUX server will run.
For example:
...
In the configuration file, a directory can be specified which contains JSON files which tell the server what application to include and where to find it on disk. The backend of these applications use the Server's Plugin structure, so much of the server-side references to applications use the term Plugin.

To include applications, be sure to define the location of the `Plugins` directory in the configuration file, through the top-level attribute _pluginsDir_
