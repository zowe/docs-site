...
- Added `zowe.sysMessages` as a property to the `Zowe YAML` configuration file. It is a new array that allows you to select messages that when found in Zowe’s job log, will be duplicated into the z/OS syslog. ([#93](https://github.com/zowe/launcher/pull/93))
