...
- `zowe.setup.mvs.proclib` shows what is the target procedure library.
...
- `zowe.setup.mvs.jcllib` is the custom JCL library. Zowe will create temporary
...
- `zowe.setup.mvs.authLoadlib` is the user custom APF LOADLIB. This field is
  optional. If this is not defined, `SZWEAUTH` from HLQ will be used as STEPLIB
  in STCs.
