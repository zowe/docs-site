...
- **Administrators and developers who need to log in to the homepage of the Discovery Service**
    ...
    - HTTPS mode: Access is protected by mainframe credentials (basic or token) or a x509 certificate.
    The `application/**` endpoint is protected by mainframe credentials (basic or token) and the `discovery/**` endpoint is protected by mainframe credentials (basic or token) or a x509 certificate.
    ...
    Some utilities including HTTPie require the certificate to be in PEM format. You can find this certificate in `keystore/localhost/localhost.pem`.
    Since the Discovery Service uses HTTPS, your client also requires verification of the validity of its certificate. Verification is performed by trusting the local CA certificate which is stored at `keystore/local_ca/localca.cer`.
...
**Note:** API Gateway first requires registration with the Discovery Service to provide authentication using mainframe credentials.
...
Provide valid mainframe credentials or a token to access the Discovery Service on localhost.
