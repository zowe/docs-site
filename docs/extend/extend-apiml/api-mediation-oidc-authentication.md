...
- The JWK Keys obtained from the authorization server's endpoint are cached for a specified interval to prevent the repeated calls to the endpoint. It can be set using the property jwks.refreshInternalHours (it's 1 hour by default)
- In subsequent calls with the same token, the Gateway reuses the cached validation outcome. As such, round trips to the OIDC / validating the jwt token(which needs fetching the keys from JWKs endpoint) are not required between short intervals when the client needs to access multiple resources in a row to complete a unit of work.
