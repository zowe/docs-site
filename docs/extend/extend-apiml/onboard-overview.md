...
- **Domain Allowlist Validation**  
    Ensure that the domain or host where your service runs is included in the API Mediation Layer's domain allowlist (`zowe.network.allowedDomains`). By default, the Discovery Service automatically trusts local environment topologies like `zowe.externalDomains`, HA instance hostnames (`zowe.haInstances.<id>.hostname`), and the `zOSMF` target hostname along with standard built-in vendor documentation domains. However, if your service utilizes an external domain outside of these defaults, the external domain must be explicitly configured. If a domain is not permitted, the service will fail to register with the Discovery Service and will remain invisible to API Gateway routing.
