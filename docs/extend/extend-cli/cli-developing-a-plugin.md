...
In order for the client-side API to reach the mainframe (to fetch data sets), Zowe CLI needs a z/OSMF profile for access. See [Using profiles](https://docs.zowe.org/stable/user-guide/https://docs.zowe.org/stable/user-guide/cli-using-using-team-profiles/) for information. 
