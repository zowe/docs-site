...
| **API Mediation Layer (APIML)** | Java | IBM Java SDK 11 (or OpenJDK 11).<br /> Java 21 is also supported in Zowe v2.18.2. and later v2.18 minor releases.<br /> Java 25 is also supported in Zowe v2.18.8. and later v2.18 minor releases.<br />| **IBM Semeru Runtime Certified Edition for z/OS, Version 17** (or OpenJDK 17) <br /> Java 21 is also supported in Zowe v3.2. and later releases. <br /> Java 25 is also supported in Zowe v3.5. and later releases. <br />| The API Mediation Layer, as a core Zowe component, requires a 64-bit Java Runtime Environment (JRE) or Java Development Kit (JDK) for z/OS. **The API Mediation Layer itself does not directly require Node.js.** |
...
:::note
Before you run Zowe on z/OS 2.5 with Java 21, ensure that you install the software update, PTF 'z/OS 2.5 PTF UI95696'. For more information, see [Java 21 users receiving CEE3728S under z/OS 2.5](https://www.ibm.com/support/pages/java-21-users-receiving-cee3728s-under-zos-25)

**z/OS OpenJ9 with Java 25**: IBM Semeru Runtime Certified Edition for z/OS provides Java 25 builds with OpenJ9. The existing OpenJ9 flags used by Zowe &mdash; `-Xquickstart` (for faster startup) and `-Xshareclasses` (for shared class cache) &mdash; continue to work on Java 25. No JVM flag changes are required when upgrading to Java 25 on z/OS.

For a full list of the latest supported versions of z/OS, Java, and Node.js, see the Zowe Compatibility Matrix in [zowe.org](https://www.zowe.org/compatibility).
