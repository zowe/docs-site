...
- `ZWED_SERVER_HTTPS_PORT`: The port used by the Zowe desktop.  Used when `LAUNCH_COMPONENT_GROUPS` includes `DESKTOP`. It should be accessible to client machines with browsers wanting to log on to the Zowe desktop.  
- `ZWES_SERVER_PORT`: This port is used by the ZSS server. Used when `LAUNCH_COMPONENT_GROUPS` includes `DESKTOP` or `ZSS`.
...
- `ZWED_SSH_PORT`: The Zowe desktop contains an application *VT Terminal* which opens a terminal to z/OS inside the Zowe desktop web page.  This port is the number used by the z/OS SSH service and defaults to 22.  The USS command `netstat -b | grep SSHD1` can be used to display the SSH port used on a z/OS system.  
...
- `ZWED_TN3270_SECURITY`: The *3270 Terminal* application needs to know whether the telnet service is using `tls` or `telnet` for security.  The default value is blank for `telnet`.
...
- `port`: Defines the port which App Server should be started on. This is equivalent to the `ZWED_SERVER_HTTPS_PORT` variable in `instance.env`.

#### Configure component zss

These configurations can be used under the `components.zss` section:

- `port`: Defines the port which ZSS should be started on. This is equivalent to the `ZWES_SERVER_PORT` variable in `instance.env`.
- `crossMemoryServerName`: Defines the procedure name of cross memory server. This is equivalent to the `ZWES_XMEM_SERVER_NAME` variable in `instance.env`.
- `tls`: Defines whether ZSS service has enabled TLS. This is equivalent to the `ZWES_SERVER_TLS` variable in `instance.env`.
