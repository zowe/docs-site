...
* By default, the API Gateway uses z/OSMF as the authentication provider. It is possible, however, to switch your authentication provider to SAF. The intended usage of SAF as an authentication provider is for systems without z/OSMF.
...
The API Gateway can output a special header that contains the value of the instance ID of the API service that the request has been routed to. This customized header is useful for understanding which service instance is being called.
