...
Each Zowe server can be customized by defining attributes within the `zowe.network.server` object of the Zowe YAML configuration file. Alternatively, the same object can be contained within an individual component's configuration, such as `components.zss.zowe.network.server` for ZSS. This option allows you to customize each component independent of other components.

Extensions are recommended to adhere to the specific configuration. Ensure that you review the documentation specific to your extension. 

### IP Addresses

By default, Zowe's servers use the TCP IP address `0.0.0.0`, which assigns the servers to be available on all network interfaces available to the jobs.

If this default is not desired, you can either change the configuration within Zowe or by setting TCPIP port assignment statements. For more information, see [IP Addresses](./address-network-requirements.md#tcpip-port-reservations) in the article _Addressing network requirements_.
...
To customize which ciphers Zowe uses, you can define a list of IANA cipher names within the Zowe YAML parameter `zowe.network.server.tls.ciphers`. For details, see this list of [IANA ciphers](https://testssl.sh/openssl-iana.mapping.html).
...
## Default and example
The default TLS configuration changes regularly as required by industry standards. The following yaml file section is an example of the defaults:
