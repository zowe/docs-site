...
<!--Consider having each certificate configuration scenario as a standalone separate article. Is there any use case under which a security administrator will perform all five scenarios in the sequence they are specified?-->
...
<!--The word 'different' does really not add any value and is not specific enough, it would be more valuable to know how many scenarios there are. -->
...
<!--as well as = and; why not just 'validate certificates?' -->
...
<!-- 'and the corresponding certificate private key' = noun+noun instead of possessive case; 'each of these objects' = which objects? -->
...
<!-- Do you use double quotes for UI field names? -->
...
<!--Can you replace Future Progressive with a different structure? 'Do you plan to use..? let=enable;-->
...
<!--This procedure is missing an intro, why do we want to use a file-based PKCS12 keystore?-->
...
4. Customize the certificate alias name. The default value is `localhost`.  
    **Note:** Use all lower case as an alias.
    <!--This note is mentioned multiple times throughout the procedure, state it once.-->
...
10. Set the domain names and IPs specified in nested subsection `SAN`. If this field is not defined, the `zwe init` command uses the value `zowe.externalDomains`.
    ...
    san:
        ...
        - 12.34.56.78
     ```
<!-- We need an outcome after the last step in the procedure-->
...
<!--This procedure is missing an intro, why do we want to import a certificate generated by another authority-->
1. Set the `type` of the certificate storage to `PKCS12`.
...
6. Set the existing PKCS12 keystore which holds the certificate issued by an external CA.  
    ...
    keystore: ""  
    ```
    <!-- this example is empty, is there any value?-->
...
<!--This procedure is missing an intro, why do we want to use Zowe generated certificates?-->
...
this field is not defined, the `zwe init` command uses the value `zowe.externalDomains`.  
    ...
    **Note**: Due to the limitation of the `RACDCERT` command, this field should contain exactly 2 entries with the domain name and IP address.
    <!--use words for numbers from one to ten; what is the result of the procedure?-->
...
<!--This procedure is missing an intro, why do we want to use existing certificates?-->
1. Set the `type` of the certificate storage to one of the following keyring types:
...
<!--Is it a good idea to keep this extended example in a blog on medium, should it be incorporated in the doc? what is the result of the procedure?-->
...
<!--This procedure is missing an intro, why do we want to import a certificate stored in a dataset?-->
1. Set the `type` of the certificate storage to one of the following keyring types:
