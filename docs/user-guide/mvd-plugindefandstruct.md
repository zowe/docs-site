# Plug-in definition and structure
...
-  **application**: Defines the plug-in as an application plug-in. Application plug-ins are composed of a collection of web content for presentation in the web component (such as the Zowe Desktop), or a collection of dataservices (REST and websocket), or both.
...
An application that has the *webContent* attribute defined provides content that is displayed in a web UI. 
...
-  **angular2**: Defines the application as having an Angular (2+) web framework component. This is the standard for a "native" framework application.

-  **iframe**: Defines the application as being external to the native web application environment, but instead embedded in an iframe wrapper.
...
Within an IFrame, the application plug-in still has access to the globals that are used by the Zowe Application Framework for application-to-application communication; simply access *window.parent.ZoweZLUX*.
