...
Dataservices are a dynamic component of the backend of an application plug-in. Dataservices are optional, because the proxy server might only serve static content for a particular application. However, when included in an application, a dataservice defines a URL space for which the server will run the extensible code from the application. Dataservices are primarily intended to be used to create REST APIs and Websocket channels.
...
An instance of a Zowe Application Framework Logger, which has its component name as the unique name of the dataservice within a plug-in.

**makeSublogger**

A function to create a Zowe Application Framework Logger with a new name, which is appended to the unique name of the dataservice.
