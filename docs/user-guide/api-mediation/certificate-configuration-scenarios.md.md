...
Zowe's servers require a keystore to store the certificates, and a truststore to validate of these certificates and any other certificates encountered.
The keystore and truststore can either be unix file-based (PKCS12) or z/OS keyring-based, when on z/OS.
Regardless of storage type, both the keystore and truststore can automatically be created by Zowe. Keystores and truststores can be populated either with certificates that the user chooses or with self-signed certificates generated by Zowe. 
...
When configuring Zowe for certificates, consider scenario best suits your use case:
