...
Users can configure some properties through environment variables in the instance.env file. Gateway service will inject them to fields. Environment variables don't have the highest priority in this context. That means, environment variable will be overridden with command line argument provided at the start of the application. Such arguments are in the start script and are prefixed with `-D`.
