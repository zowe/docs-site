...
Not every configuration property is exposed via zowe vide UPPER_CASE named variables provided in the instance.env. However, users can configure those too. Provided correct pattern, API ML services will inject them into fields.

One use case can be to provide label for JWT secret. This secret is usualy generated with zowe installation. In some scenarios, users want to use their own secret which is already created and labled differently. 
...
* **apiml.security.auth.jwtKeyAlias**
    ...
    Label, under which the secret that is used to sign JWT, is stored in keystore or keyring. Default value is `jwtsecret`. The secrete generated by zowe install script is stored under label `jwtsecret`. 
