# Overview of API ML Observability and OpenTelemetry Architecture
...
By adopting the OpenTelemetry standard, API ML provides a vendor-neutral way to monitor performance and diagnose issues. System administrators can export this data to industry-standard analysis tools, commonly referred to as an OpenTelemetry (OTel) Collector, to gain insights into resource utilization and latency within the mainframe environment.
...
This contextual automation ensures that telemetry data remains traceable to the specific physical or logical mainframe partition where the service is running.
...
1. **The Provider (API ML):** Captures internal events and exports them using the OTLP protocol.
2. **The Collector:** A standalone service (OTel Collector) that receives, processes, and exports data.
3. **The Backend (Visualization):** Tools like Grafana, Jaeger, or Prometheus where the data is stored and visualized.

## Next Steps

* To begin your configuration, see [Configuring API ML Observability](configuring-apiml-observability.md).
* To understand the specific metrics and traces produced, see [Telemetry Signals in API ML](telemetry-signals.md).
