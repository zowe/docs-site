...
**Note:** Change the following property if user mapping is provided by external API:
...
**Note:** The following property is only required if the Zowe runtime userId is altered from the default `ZWESVUSR`:
