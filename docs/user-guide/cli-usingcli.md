...
View the [CLI Command Reference Guide (PDF)](../CLIReference_Zowe.pdf) for a detailed documentation on commands, actions, and options in Zowe CLI. It is also available as a [ZIP file](../zowe_web_help.zip) containing HTML help viewable in a web browser. The reference document is based on the `@lts-incremental` version of the CLI.
