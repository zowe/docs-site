...
The Zowe command line interface (CLI) can be used interactively from a command window on any computer on which it has been installed.  If you are wishing to use the CLI together with a screen reader providing accessibility it is recommended that for a Mac™ use the Terminal application and enable it for Accessibility through [System Preferences > Accessibility](https://support.apple.com/zh-sg/guide/terminal/trml1020/mac), and on a Windows™ PC use the command prompt.  For other operating systems or for using alternative terminals check the specification for the terminal used to ensure it meets accessibility requirements.  
