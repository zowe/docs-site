...
-   *httpsPort* is the port number that is assigned to *node.https.port* in `zluxserver.json`.
    For example, if the Zowe Node Server runs on host *myhost* and the port number that is assigned to *node.http.port* is 12345, you specify `https://myhost:12345/ZLUX/plugins/com.rs.mvd/web/index.htm`.
