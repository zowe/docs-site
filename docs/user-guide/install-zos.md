...
4. Execute the `zowe-install.sh` script to start installation. 
    ...
    **Note:** You might receive the following error that the file cannot be executed:
    ...
    When the `zowe-install.sh` script runs, it performs a number of steps. For more details about these steps, see [How the install script `zowe-install.sh` works](#how-the-install-script-zowe-installsh-works).
