...
5. Security requirements for the cross memory server
    ...
    - If you use RACF, issue the following commands:
        ...
        ```
        - To check whether the permission has been successfully granted run the command
        ```
        RLIST FACILITY ZWES.IS AUTHUSER
        ```
        This will show the user IDs who have access to the ZWES.IS class, which should include IZUSVR with READ access.
